...
export class ProductDuplicateService {
  ...
  constructor(
    ...
    this.handleError = httpErrorHandler.createHandleError('ProductDuplicateService');
  }

  ngOnInit() {}

  duplicateProduct (product: Product): Observable<Product> {
    return this.http.post<Product>(this.apiUrl, product, { headers: new HttpHeaders({'Content-Type':  'application/json'}) })
      .pipe(
        ...
        catchError(this.handleError('duplicateProduct', product))
      );
  }

  private log(message: string) {
    this.apimessageService.add(`ProductDuplicateService: ${message}`);
  }
